...
import { GetStateResponseSchema, UnlockWalletResponseSchema, WalletState, GetInfoResponseSchema, ListChannelsResponseSchema, ListPeersResponseSchema, WalletBalanceResponseSchema, SendCoinsResponseSchema, SendCoinsRequestSchema, NewAddressRequestSchema, NewAddressResponseSchema, ChannelBalanceResponseSchema, OpenChannelRequestSchema, ChannelPointSchema, CloseChannelRequestSchema, CloseStatusUpdateSchema, PendingUpdateSchema, ChannelCloseUpdateSchema, PendingChannelsResponseSchema, PendingChannelsRequestSchema } from "../proto/lightning_pb.js";
import { SubscribeStateResponseSchema } from "../proto/stateservice_pb.js";
import { mockPeers, mockChannels, mockWalletBalance, mockGetInfoResponse, generateMockAddress, mockChannelBalanceResponse, generateMockTxid, mockPendingChannelsResponse } from "./responses.js";
let currentState = WalletState.LOCKED;
...
const callSubscribeStateOnResponseCallbacks = () => {
  subscribeStateOnResponseCallbacks.forEach(cb => {
    cb.onResponse(base64Encode(toBinary(GetStateResponseSchema, create(GetStateResponseSchema, {
      state: currentState
    }))));
  });
};
const TurboLnd = {
  ...
  subscribeState: (_, onResponse, onError) => {
    ...
    setTimeout(() => {
      ...
      const state = toBinary(SubscribeStateResponseSchema, create(SubscribeStateResponseSchema, {
        state: currentState
      }));
  ...
  initWallet: async _ => {
    ...
    if (currentState === WalletState.LOCKED) {
      return base64Encode(toBinary(UnlockWalletResponseSchema, create(UnlockWalletResponseSchema, {})));
    }
  ...
  listChannels: async _ => {
    return base64Encode(toBinary(ListChannelsResponseSchema, create(ListChannelsResponseSchema, {
      channels: mockChannels
    })));
  },
  pendingChannels: async request => {
    fromBinary(PendingChannelsRequestSchema, base64Decode(request));
    return base64Encode(toBinary(PendingChannelsResponseSchema, mockPendingChannelsResponse));
  },
  listPeers: async _ => {
    return base64Encode(toBinary(ListPeersResponseSchema, create(ListPeersResponseSchema, {
      peers: mockPeers
    })));
  },
  walletBalance: async _ => {
    return base64Encode(toBinary(WalletBalanceResponseSchema, mockWalletBalance));
  },
  channelBalance: async _ => {
    const response = create(ChannelBalanceResponseSchema, mockChannelBalanceResponse);
    return base64Encode(toBinary(ChannelBalanceResponseSchema, response));
  },
  newAddress: async request => {
    const decodedRequest = fromBinary(NewAddressRequestSchema, base64Decode(request));
  ...
  sendCoins: async request => {
    const decodedRequest = fromBinary(SendCoinsRequestSchema, base64Decode(request));
  ...
  openChannelSync: async request => {
    const decodedRequest = fromBinary(OpenChannelRequestSchema, base64Decode(request));
  ...
  closeChannel: (request, onResponse, onError) => {
    const decodedRequest = fromBinary(CloseChannelRequestSchema, base64Decode(request));
